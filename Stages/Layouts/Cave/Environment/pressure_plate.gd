class_name PressurePlate
extends Node2D
## Placa de presión. Se activa mientras haya al menos un cuerpo (Caja o Jugador)
## sobre su InteractionArea y se desactiva solo cuando el último se retira.
## Cambia de frame y notifica su estado con la señal plate_activated.

## Emitida al activarse (true) o desactivarse (false) la placa.
signal plate_activated(activated: bool)

# Frames del Sprite2D (hframes = 2): 0 = sin presionar, 1 = presionada.
const FRAME_INACTIVE: int = 0
const FRAME_ACTIVE: int = 1

@onready var sprite: Sprite2D = $Sprite2D
@onready var interaction_area: Area2D = $InteractionArea

# Contador de cuerpos sobre la placa: evita desactivarla si queda alguno encima.
var _bodies_on_plate: int = 0

func _ready() -> void:
	sprite.frame = FRAME_INACTIVE
	interaction_area.body_entered.connect(_on_body_entered)
	interaction_area.body_exited.connect(_on_body_exited)

## Es el flanco de subida (primer cuerpo) el que activa la placa.
func _on_body_entered(_body: Node2D) -> void:
	_bodies_on_plate += 1
	if _bodies_on_plate == 1:
		sprite.frame = FRAME_ACTIVE
		plate_activated.emit(true)

## Solo el último cuerpo en salir desactiva la placa.
func _on_body_exited(_body: Node2D) -> void:
	_bodies_on_plate = max(_bodies_on_plate - 1, 0)
	if _bodies_on_plate == 0:
		sprite.frame = FRAME_INACTIVE
		plate_activated.emit(false)
