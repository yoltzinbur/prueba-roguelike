...
- `reset_save()` — **NEW GAME**: borra `savegame.cfg` del disco y devuelve TODO el estado en memoria
  a sus valores por defecto (incluido `GameManager.coins = 0`). Como `SaveManager` es autoload, borrar
  solo el archivo no basta (el estado ya esta cargado), por eso reinicia ambos. La invoca el boton
  NEW GAME de `MainMenu.gd` antes de entrar al router `Boot` (que, sin progreso, lleva a la cueva
  inicial). El boton START normal entra a `Boot` sin borrar nada (reanuda la partida guardada).
