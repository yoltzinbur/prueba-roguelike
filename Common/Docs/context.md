...
- **Input centralizado:** Solo `PlayerInputComponent` y `EnemyInputComponent` pueden leer de `Input`. Los objetos del entorno (`Stages/`, `Entities/Chest/`, etc.) nunca llaman a `Input.is_action_just_pressed()` directamente; usan el sistema de interacciones. Acciones definidas en `project.godot` incluyen `parry` (clic derecho), usada por el estado `Parry`.
