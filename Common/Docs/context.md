...
- **Parry / Guard Break:** con la accion `parry` (clic derecho) entra al estado `Parry`, de doble funcion: refleja los **proyectiles** del jefe y, como **escudo**, anula el dano del melee de enemigos normales y los **empuja** (los mete en su estado `Hit`). El melee de los **jefes** (grupo `Boss`) atraviesa el escudo. `on_projectile_reflected()` cuenta reflejos y cada 3 dispara un Guard Break (aturde al jefe + abre ventana de critico); `consume_crit_multiplier()` aplica el ×3 al siguiente golpe. Ver `architecture.md`.
